 ...
 */
function getParticipantContext(participantId: string, dekkoIv: Uint8Array, dekkoKey: CryptoKey): Context {
    if (sharedContext) {
...
onmessage = (event: MessageEvent<IWorkerMessageEvent>) => {
    ...
    if (operation === 'initialize') {
        ...
        const { participantId, key, keyIndex, dekkoIv, dekkoKey } = event.data;
...
if (self.RTCTransformEvent) {
    self.onrtctransform = (event: IRTCTransformerEvent) => {
        ...
        const { operation, participantId, dekkoIv, dekkoKey } = transformer.options;
        const context = getParticipantContext(participantId, dekkoIv, dekkoKey);
